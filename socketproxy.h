...
enum socket_type {
    ...
    RILD_SYNC,
    UIM_LPA_SOCKET,
    UIM_REMOTE_CLIENT_SOCKET,
    UIM_REMOTE_SERVER_SOCKET,
};
...
extern int recv_inner_fd(int conn_fd);

extern int send_inner_fd(int conn_fd, int fd);

extern int send_buf(int fd, void *data, size_t len);

extern void _create_pthread(void *(*start_routine)(void *), void *arg);

extern void thread_started(void);

extern	int create_inner_socket(int index, const char *name);

extern int start_listen_inner(slp *s_lp);

extern int connect_inner_socket(int index, const char *name);

extern void *rilproxy_exit_handler(void *unused);

extern void rilproxy_sigchld_handler(int sig);

extern void handle_sigchld_sig(void);

extern void set_parent_pid(pid_t p_pid);
#endif
