...
static pthread_t s_tid_event_loop;
static pthread_t s_tid_conn_server;
...
int send_buf(int fd, void *data, size_t len)
{
	int ret, remain = len;
	char *buf = data;
	while (remain > 0) {
...
int recv_len_data(int fd, char *buf, int length)
{
    ...
    do {
        ...
        if (count_read <= 0 ) {
            RLOGE("%s Hit EOS reading message length strerr:%s count_read:%d", __func__, strerror(errno), count_read);
            return -1;
        }
...
{
	if (c_sock[0].fission_mode == FISSION_MODE_DOUBLE) {
...
static void trigger_ev_loop() {
    int ret;
    if (!pthread_equal(pthread_self(), s_tid_event_loop)) {
...
)
{
    ...
    RLOGV("%s %s:%s fd:%d fd:%d", __func__, 
	...
	for (;;) {
		...
		if (ret == 0 && p_record == NULL) {
			...
			if (!s_sock->wait && s_sock->s_cp->cache) {
				if (s_sock->fission_mode == FISSION_MODE_DOUBLE) {
...
)
{
    ...
    RLOGI("accept %s %s:%s fdCommand:%d", __func__, c_sock->sub_dir, *(c_sock->sock_name + c_sock->index), fd);
...
void _create_pthread(void *(*start_routine)(void *), void *arg, pthread_t *thread_id)
{
    ...
    int result = pthread_create(thread_id, &attr, start_routine, arg);
    if (result != 0) {
...
{
    _create_pthread(event_loop, NULL, &s_tid_event_loop);
}
...
)
{
    ...
    RLOGI("%s %s:%s", __func__, c_sock->sub_dir, sock);
...
static void *connect_server(void *param)
{
    ...
    if ((s_sock->type == QCRIL_RADIO_CONFIG) || (s_sock->type == UIM_LPA_SOCKET)
		|| (s_sock->type == UIM_REMOTE_CLIENT_SOCKET) || (s_sock->type == UIM_REMOTE_SERVER_SOCKET)
		|| (s_sock->type == RILD_IMS) || (s_sock->type == RILD) || (s_sock->type == RILD_OEM)) {
        _connect_server(s_sock, 1);
    } else {
        if (s_sock->fission_mode == FISSION_MODE_DOUBLE) {
            _connect_server(s_sock, 2);
        } else {
            _connect_server(s_sock, 1);
        }
    }
...
)
{
    ...
    _create_pthread(connect_server, (void *)s_sock, &s_tid_conn_server);
}
...
static int send_fd(int conn_fd, int fd)
{
	struct msghdr msg;
	struct iovec iov[1];
	char c = 'x';
...
re_recv:
	...
	if ((fd <= 0) && (errno == EAGAIN || errno == EINTR)) {
		RLOGD("%s re-recv errno:%d strerr:%s\n", __func__, errno, strerror(errno));
		sleep(1);
		goto re_recv;
	} else if (fd <= 0) {
		RLOGE("%s error errno:%d strerr:%s\n", __func__, errno, strerror(errno));
		close(conn_fd);
		return -1;
	}
	return fd;
...
int send_inner_fd(int conn_fd, int fd)
{
	int ret = -1;
	
re_send:
	ret = send_fd(conn_fd, fd);
	if ((ret < 0) && (errno == EAGAIN || errno == EINTR)) {
		RLOGD("send_internal_fd re-send errno:%d strerr:%s\n", errno, strerror(errno));
		sleep(1);
		goto re_send;
	} else if (ret < 0) {
		RLOGE("send_internal_fd error errno:%d strerr:%s\n", errno, strerror(errno));
		close(fd);
		return -1;
	}
...
int create_inner_socket(int index, const char *name)
{
	struct sockaddr_un addr;
	int fd, ret;
	...
	snprintf(addr.sun_path, sizeof(addr.sun_path), "/dev/socket/%s%d", name, index);
	ret = unlink(addr.sun_path);
	if (ret != 0 && errno != ENOENT) {
		RLOGE("Failed to unlink old socket %s\n", strerror(errno));
		goto out_close;
	}

	ret = bind(fd, (struct sockaddr *) &addr, sizeof (addr));
	if (ret) {
		RLOGE("Failed to bind socket %s\n", strerror(errno));
		goto out_unlink;
	}

	chown(addr.sun_path, AID_ROOT, AID_RADIO);
	chmod(addr.sun_path, S_IRUSR|S_IWUSR|S_IRGRP|S_IWGRP);

	RLOGI("Created socket %s fd:%d", addr.sun_path, fd);
...
static void listen_inner_client_callback(int fd, short flags, void *param)
{
    slp *s_lp = (slp *)param;
    struct sockaddr_un remote;
	socklen_t len;
    ...
    fd_command = accept(fd, (struct sockaddr *)&remote, &len);
    if (fd_command < 0) {
		RLOGE("default_listen_debug_callback accept: %s", strerror(errno));
		return;
	}

    RLOGI("accept %s fdCommand:%d", __func__, fd_command);

    s_lp->fd_command = fd_command;

    //s_lp->process_commands_callback = handle_inner_socket_recv_message;

    socket_event_set(s_lp->commands_event, fd_command, true, s_lp->process_commands_callback, s_lp);
    socket_event_add_wakeup(s_lp->commands_event);
}

int start_listen_inner(slp *s_lp)
{
	int ret;
    
    ret = listen(s_lp->fd_listen, 4);
    if (ret < 0) {
        RLOGE("Failed to listen on control socket '%d' %s", s_lp->fd_listen, strerror(errno));
        exit(-1);
    }

    RLOGI("%s", __func__);

    s_lp->listen_client_callback = listen_inner_client_callback;

    socket_event_set(s_lp->listen_event, s_lp->fd_listen, false, s_lp->listen_client_callback, s_lp);
...
int connect_inner_socket(int index, const char *name)
{
	char path[256];
	int fd;

	snprintf(path, sizeof(path), "/dev/socket/%s%d", name, index);
	while (!socket_exists(path)) {
		sleep(1);
	}
	...
	RLOGI("%s %d fd:%d", __func__, __LINE__, fd);

	return fd;
}

void *rilproxy_exit_handler(void *unused)
{
	int status;
	int ret;
	
	ret = waitpid(pid_parent, &status, 0);
	RLOGI("waitpid status:%d, ret:%d.", status, ret);
	
	exit(EXIT_FAILURE);
}

void rilproxy_sigchld_handler(int sig)
{
	pthread_t tid;
	
	pthread_create(&tid, NULL, rilproxy_exit_handler, NULL);
}


void handle_sigchld_sig(void)
{
	int ret;
	...
	sigact.sa_handler = rilproxy_sigchld_handler;
	sigact.sa_flags = SA_NOCLDSTOP;
	ret = sigaction(SIGCHLD, &sigact, NULL);
	if (ret < 0) {
		RLOGE("SIGCHLD sigaction failed ret:%d\n", ret);
	}
}

void set_parent_pid(pid_t p_pid)
{
	pid_parent = p_pid;
}
